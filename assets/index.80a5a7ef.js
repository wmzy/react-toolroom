...
const scriptRel = 'modulepreload';const assetsURL = function(dep) { return "/react-toolroom/"+dep };const seen = {};const __vitePreload = function preload(baseModule, deps, importerUrl) {
    // @ts-ignore
...
function createStateContext() {
  ...
  var ValueContext = /*#__PURE__*/react.exports.createContext(undefined);
  var SetterContext = /*#__PURE__*/react.exports.createContext(undefined);
  function Provider(_ref) {
    var children = _ref.children;
    var _useState = react.exports.useState.apply(void 0, _toConsumableArray$1(params)),
...
var ViewContext = /*#__PURE__*/react.exports.createContext(null);
function ViewProvider(props) {
  return /*#__PURE__*/jsx(ViewContext.Provider, _objectSpread$4({}, props));
}
function useView() {
...
var DataContext = /*#__PURE__*/react.exports.createContext([undefined, {}]);
...
function DataProvider(_ref) {
  var children = _ref.children,
    name = _ref.name,
    data = _ref.data;
  var namedData = useNamedData();
  return /*#__PURE__*/jsx(DataContext.Provider, {
    value: [data, name ? _objectSpread$4(_objectSpread$4({}, namedData), {}, _defineProperty$4({}, name, data)) : namedData],
    children: children
  });
}
var LoadingContext = createStateContext();
function useLoading() {
...
function createCurrentGuard() {
  ...
  return [function currentGuard(promise) {
    ...
    return promise.then(function (result) {
      return current === cur ? result : cancelPromise();
    }).catch(function (err) {
      return current === cur ? Promise.reject(err) : cancelPromise();
    });
  }, function cancel() {
    current = undefined;
  }];
...
function create(routes, history, resolveView, options) {
  var _createCurrentGuard = createCurrentGuard(),
    _createCurrentGuard2 = _slicedToArray$3(_createCurrentGuard, 2),
    currentGuard = _createCurrentGuard2[0],
    cancelAll = _createCurrentGuard2[1];
...
function match(router, pathname) {
  ...
  pathname) {
    for (var i = 0; i < routes.length; i++) {
      ...
      var matched = route.path ? match$1(route.path, {
        strict: true,
        sensitive: true,
      ...
      if (matched) {
        var result = _objectSpread$3({
          route: route
        }, matched);
        if (end) return [result];
        var children = matchRoutes(route.children, "".concat(baseUrl).concat(route.path || ''), pathname.slice(matched.path.length));
        if (children) return [result].concat(_toConsumableArray(children));
        return undefined;
      }
    }
    return undefined;
...
function resolve(router, location) {
  var matched = match(router, location.pathname);
  if (!matched) return Promise.reject(new Error('Not Found'));
  var resolveView = router.resolveView;
...
function commit(router, resolvePromise, location) {
  ...
  var history = router.history,
    currentGuard = router.currentGuard,
    _router$onLoadingChan = router.onLoadingChange,
    onLoadingChange = _router$onLoadingChan === void 0 ? noop : _router$onLoadingChan;
  var nextIndex = (((_history$location$sta = history.location.state) === null || _history$location$sta === void 0 ? void 0 : _history$location$sta.index) || 0) + 1;
  if (router.resolving) {
    onLoadingChange();
  }
  router.resolving = location;
  onLoadingChange('pending');
  return currentGuard(resolvePromise).then(function (resolvedView) {
    router.locationStack = [].concat(_toConsumableArray(router.locationStack.slice(0, nextIndex)), [location]);
    router.viewStack = [].concat(_toConsumableArray(router.viewStack.slice(0, nextIndex)), [resolvedView]);
...
function commitReplace(router, resolvePromise, location) {
  var _history$location$sta2;
  var history = router.history,
    currentGuard = router.currentGuard,
    _router$onLoadingChan2 = router.onLoadingChange,
    onLoadingChange = _router$onLoadingChan2 === void 0 ? noop : _router$onLoadingChan2;
  var index = ((_history$location$sta2 = history.location.state) === null || _history$location$sta2 === void 0 ? void 0 : _history$location$sta2.index) || 0;
  if (router.resolving) {
    onLoadingChange();
  }
  router.resolving = location;
...
function navigate(router, to, state) {
  var location = toLocation(router, to, state);
  var viewPromise = resolve(router, location);
...
function createHref(_ref, to) {
  var baseUrl = _ref.baseUrl,
    history = _ref.history;
  return baseUrl + history.createHref(to);
}
function cancel(_ref2) {
  var cancelAll = _ref2.cancelAll,
    _ref2$onLoadingChange = _ref2.onLoadingChange,
    onLoadingChange = _ref2$onLoadingChange === void 0 ? noop : _ref2$onLoadingChange;
  cancelAll();
  onLoadingChange();
}
function listen(router, onViewChange) {
  var _history$location$sta3;
  var _router$onLoadingChan3 = router.onLoadingChange,
    onLoadingChange = _router$onLoadingChan3 === void 0 ? noop : _router$onLoadingChan3,
    history = router.history;
  var locationStack = ((_history$location$sta3 = history.location.state) === null || _history$location$sta3 === void 0 ? void 0 : _history$location$sta3.locationStack) || [history.location];
  router.locationStack = locationStack;
  onLoadingChange('pending');
  Promise.all(locationStack.map(function (l) {
    return resolve(router, l);
  })).then(function (views) {
  ...
  var rmListener = history.listen(function (_ref3) {
    var _location$state;
    var action = _ref3.action,
      location = _ref3.location;
    cancel(router);
    var index = ((_location$state = location.state) === null || _location$state === void 0 ? void 0 : _location$state.index) || 0;
    onViewChange(router.viewStack[index]);
    if (action === 'POP') {
      history.replace(createPath(history.location), _objectSpread$3(_objectSpread$3({}, history.location.state), {}, {
        locationStack: router.locationStack
      }));
    }
  });
...
function resolveView(matched, _ref) {
  var router = _ref.router,
    location = _ref.location;
  return Promise.all(matched.map(function (_ref2, index) {
    var _route$data;
    ...
    function resolveComponent() {
      ...
      var r = route.component(params, ctx);
      return Promise.resolve(r).then(function (m) {
        return 'default' in m ? m.default : m;
      });
    }
    return Promise.all([(_route$data = route.data) === null || _route$data === void 0 ? void 0 : _route$data.call(route, params, ctx), resolveComponent()]).then(function (_ref3) {
    ...
    return views.reverse().reduce(function (acc, view) {
      return /*#__PURE__*/jsx(ViewProvider, {
        value: acc,
        children: view
      });
    });
...
var _excluded$2 = ["initialEntries", "initialIndex"];
...
var RouterContext = /*#__PURE__*/react.exports.createContext(null);
function BRouter(_ref) {
  var routes = _ref.routes,
    history = _ref.history,
    children = _ref.children,
    _ref$errorHandler = _ref.errorHandler,
    errorHandler = _ref$errorHandler === void 0 ? Promise.reject : _ref$errorHandler,
    baseUrl = _ref.baseUrl,
    _ref$resolveView = _ref.resolveView,
    resolveView$1 = _ref$resolveView === void 0 ? resolveView : _ref$resolveView;
  var setLoading = useLoadingSetter();
  var router = react.exports.useMemo(function () {
    return create(routes, history, resolveView$1, {
      baseUrl: baseUrl,
      errorHandler: errorHandler,
      onLoadingChange: function onLoadingChange(status) {
        setLoading(status && {
          key: uniqId(),
          status: status
        });
...
function BaseRouter(props) {
  return /*#__PURE__*/jsx(LoadingContext.Provider, {
    children: /*#__PURE__*/jsx(BRouter, _objectSpread$2({}, props))
  });
}
function RouterComponent(props) {
  var history = createBrowserHistory();
  return /*#__PURE__*/jsx(BaseRouter, _objectSpread$2(_objectSpread$2({}, props), {}, {
    history: history
  }));
}
...
function Link(_ref) {
  ...
  var router = useRouter(); // TODO: 防重复点击
  ...
  return /*#__PURE__*/(
    ...
    jsx("a", _objectSpread$1(_objectSpread$1({}, rest), {}, {
...
function App() {
  ...
  return /*#__PURE__*/jsxs(RouterComponent, {
    routes: routes
    // baseUrl={"/react-toolroom/".slice(0, -1)}
    ,
    baseUrl: "/react-toolroom/"?.slice(0, -1),
    errorHandler: e => /*#__PURE__*/jsx(RouterError, {
      error: e
...
export { Link as L, View as V, jsx as a, jsxs as j, react as r };
