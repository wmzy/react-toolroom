...
- **Zero dependencies, tiny footprint** — `react-toolroom` is 1.38 kB and `react-toolroom/async` is 2.61 kB (minified + brotli, including the shared chunk), enforced by CI budgets of 2 kB / 4 kB.
...
- **TypeScript first** — authored in TypeScript, `.d.ts` generated from source; 130 tests.
...
| Invalidation linked to mutations | ✗ (manual `cache.delete` / `cache.clear`) | `invalidateQueries` | manual `mutate` | manual |
...
| Bundle size¹ | **1.38 kB** + **2.61 kB** | ≈ 13 kB | ≈ 4 kB | ≈ 5 kB+ |
...
**Choose react-toolroom** for small-to-mid applications, for embedding inside a component library, or when you only want to cherry-pick a few capabilities at minimal cost. **Choose TanStack Query** when you need complete server-state management — mutation/invalidation linkage, pagination, SSR hydration, DevTools.
...
`usePolling` skips a tick while the previous call is still pending (a slow API never piles up concurrent requests) and pauses while the document is hidden unless you pass `{whenHidden: true}`. `useFocusRevalidate` throttles with `{interval}` (default `0`).
...
| `useRun(fn, args, options?)` | Run `fn(...args)` on mount and whenever `args` change. `{signal: true}` appends an `AbortSignal` as the last argument and aborts it on change/unmount. |
| `useCache(fn, cacheProvider, staleTime = 0)` | SWR caching: cache hits broadcast immediately; stale entries revalidate in the background. Returns whether the current data is stale. |
...
| `usePolling(fn, interval, {whenHidden = false}?)` | Call `fn()` every `interval` ms; skips ticks while the previous call is pending; pauses while the document is hidden unless `whenHidden`. Changing `interval` restarts the timer. |
| `useFocusRevalidate(fn, {interval = 0}?)` | Refetch on window focus and on `visibilitychange` back to visible, throttled by `interval`. |
...
- **CI size budgets** — [size-limit](./.size-limit.json) keeps `react-toolroom` under 2 kB and `react-toolroom/async` under 4 kB (brotli, entry + shared chunk). Currently 1.38 kB / 2.61 kB.
...
- **Tested** — 130 tests (vitest + Testing Library).

## Demos

See [demos](./demos/) for runnable examples: `memo`, request deduplication, polling, focus revalidation, SWR caching, and cancellation via `AbortSignal`.
